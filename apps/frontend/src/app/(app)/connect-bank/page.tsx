...
export default function ConnectBankPage() {
  ...
  const [uploadStatus, setUploadStatus] = useState<'idle' | 'queued' | 'error'>('idle');
    ...
    try {
      const { jobId } = await api.post<{ jobId: string; status: string }>(
        API_ENDPOINTS.scrape.start,
        {
          ...
          // Credentials are sent as-is here; in production they would be
          // encrypted with the API's RSA-OAEP public key (WebCrypto) before leaving the browser.
  ...
  async function handleUpload() {
    ...
    try {
      await api.post(API_ENDPOINTS.scrape.start, {
        bankName: 'manual_upload',
    ...
    <div className="space-y-6">
          ...
          return (
            <Card key={bank.id}>
              ...
              <CardContent className="space-y-3">
                ...
                {/* Inline credential form when dialog is open for this bank */}
                {openDialog === bank.id && (
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <p className="text-sm text-muted-foreground">
            Upload your PDF or CSV bank statements and we will extract your transaction history.
          </p>
          {uploadStatus === 'queued' && (
            <p className="text-sm text-muted-foreground">✓ Upload job queued</p>
          )}
          {uploadStatus === 'error' && (
